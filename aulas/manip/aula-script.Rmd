...
mutate(
  ...
  flight_time = ifelse(
    ...
    (24*60 - dep_time_min) + arr_time_min 
    )
  )
```
...
flights %>%
  ...
  mutate(
    ...
    sched_dep_time2 = sched_dep_time_min + dep_delay
  ) %>%
  select(sched_dep_time_min, sched_dep_time2)
```
...
```{r}
flights %>%
  filter(month == 1, arr_delay > 180) %>%
  select(carrier, dest, ends_with("delay")) %>%
...
flights %>%
  summarise(
    qtd_atrasos_2h = sum(arr_delay > 120),
    mediana_atraso = median(arr_delay),
    media_atraso = mean(arr_delay[arr_delay > 0]),
    qtd_adiantado = sum(arr_delay < 0)
  )
```
...
3) A definição de "não cancelado" não está totalmente correta? Por que? Qual é a coluna mais importante? (Não precisa fazer agora)
